...
// format data into sub arrays of three measurements
for (const val of data) {
    windows.push([val])

    if (idx === 1) {
        windows[idx - 1].push(val)
    }

    if (idx >= 2) {
        windows[idx - 1].push(val)
        windows[idx - 2].push(val)
    }

    ++idx
}

const result = windows.reduce((acc, curVal, idx) => {
    if (idx === 0) return acc

    if (curVal.length === 3 && windows[idx - 1].length === 3) {

        const a = windows[idx - 1].reduce((acc, curVal) => acc += curVal)
        const b = curVal.reduce((acc, curVal) => acc += curVal) 
...
console.log(`Based on sums of a three measurement sliding window, depth increased ${result} times`)