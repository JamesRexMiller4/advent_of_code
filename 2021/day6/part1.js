...
lanternfish might have 2 days left until it creates another lanternfish, 
...
const passTime = (days, fishBowl) => {
    while (days > 0) {
        fishBowl.forEach((fish) => {
            fish.decrementTimer();
        })
        --days;
    }
...
starterFish.forEach(fish => {
    const fishy = new LanternFish(fish);
    fishBowl.push(fishy);
})

passTime(80, fishBowl);

console.log(fishBowl.length)