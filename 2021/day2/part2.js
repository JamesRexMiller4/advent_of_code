...
you would have after following the planned course. What do you get if you multiply your final horizontal 
position by your final depth?
*/

const fs = require("fs")
const data = fs.readFileSync("./puzzleInput.txt").toString().split("\n");
const result = { pos: 0, depth: 0 };
let aim = 0;

for (const direction of data) {
    const [dir, val] = direction.split(" ")
    
    switch (dir) {
        case "forward":
            result.pos += +val
            result.depth += (aim * +val)
            break;
        case "up":
            aim -= +val
            break;
...
console.log(result)
console.log(`Multiplying final position by final depth equals ${result.pos * result.depth}`)